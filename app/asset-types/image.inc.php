...
Class Image extends Asset {
  ...
  function set_extended_data($file_path) {
    ...
    $this->data['shutter'] = $exif["EXIF"]["ExposureTime"];
    $this->data['aperture'] = $exif["COMPUTED"]["ApertureFNumber"];
    $this->data['iso'] = 'ISO ' . $exif["EXIF"]["ISOSpeedRatings"];
    $this->data['camera_make'] = $exif["IFD0"]["Make"];
    $this->data['camera'] = $exif["IFD0"]["Model"];
    $this->data['focal'] = round($Focal) . 'mm';
    $this->data['datetime'] = $exif["EXIF"]["DateTimeOriginal"];
  }
