...
Class PageData {
  ...
  static function get_featured_image($file_path) {
    $featuredImages = array_keys(Helpers::list_files($file_path, '/featured-image\.(gif|jpg|png|jpeg)$/i', false));
    if (!empty($featuredImages)) {
      $featured_image = new Image($file_path.'/'.$featuredImages[0]);
  ...
  static function get_hero_image($file_path) {
    $heroImage = array_keys(Helpers::list_files($file_path, '/hero-image\.(gif|jpg|png|jpeg)$/i', false));
    if (!empty($heroImage)) {
      $hero_image = new Image($file_path.'/'.$heroImage[0]);
      return $hero_image->data;
    } else {
      return false;
    }
  }
  ...
  static function get_revindex($siblings, $file_path, $total_siblings) {
    $count = $total_siblings + 1;
    if(!empty($siblings)) {
      foreach($siblings as $sibling) {
        $count--;
        # return current count value
  ...
  static function get_file_types($file_path) {
    ...
    foreach(Helpers::list_files($file_path, '/\.[\w\d]+?$/', false) as $filename => $file_path) {
      preg_match('/(?<!thumb|featured-image|_lge|_sml)\.(?!yml)([\w\d]+?)$/', $filename, $ext);
  ...
  static function create_vars($page) {
    ...
    # page.featured_image
    $page->featured_image = self::get_featured_image($page->file_path);
    # page.hero_image
    $page->hero_image = self::get_hero_image($page->file_path);
    # page.current_year
    ...
    # page.revindex
    $page->revindex = self::get_revindex($page->data['siblings_and_self'], $page->file_path, $page->data['siblings_count']);
  ...
  static function create_collections($page) {
    ...
    # page.paginated_items
    $items_per_page = 10;
    $max_pages = ceil(strval(count($page->data['children'])) / $items_per_page );
    $page_start = 0;

    if (isset($_GET['page'])) {
      $page_start = $_GET['page'] - 1;
    } else {
      $page_start = 0;
    }

    $page->paginated_items = array_slice(array_reverse($page->data['children']), $items_per_page * $page_start, $items_per_page, true);

    #page pagination links
    $get_page = (isset($_GET['page'])) ? (int)$_GET['page'] : 1;

    if ($get_page > 1) {
      $page->next_page = '?page='.($get_page - 1);
    } else {
      $page->next_page = null;
    }
    if ($get_page < $max_pages ) {
      $page->prev_page = '?page='.($get_page + 1);
    } else {
      $page->prev_page = null;
    }
  ...
  static function create_asset_collections($page) {
    # page.files
    $page->files = Helpers::list_files($page->file_path, '/(?<!thumb|featured-image|hero-image|_lge|_sml)\.(?!yml)([\w\d]+?)$/i', false);
    # page.images
    $page->images = Helpers::list_files($page->file_path, '/(?<!thumb|featured-image|hero-image|_lge|_sml)\.(gif|jpg|png|jpeg)$/i', false);
    # page.video
